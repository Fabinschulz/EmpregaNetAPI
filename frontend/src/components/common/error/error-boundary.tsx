...
> {
  ...
  render() {
    ...
    const { children, status, fallback, statusCode, handleOnButton } =
      this.props;
          ...
          <Card className={styles.card}>
            <CardHeader className={styles.cardHeader}>
              ...
              <h4 className={styles.bannerTitle}>
                O serviço{" "}
                <span className={styles.bannerStrong}>{fallback ?? ""}</span>{" "}
                encontra-se temporariamente indisponível no momento.
              </h4>
              <CardTitle>Algo deu errado</CardTitle>
              <CardDescription>
                Ocorreu um erro ao tentar carregar as informações. Por favor,

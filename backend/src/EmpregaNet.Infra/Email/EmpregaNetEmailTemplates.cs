...
internal static class EmpregaNetEmailTemplates
{
    ...
    public static (string Subject, string HtmlBody) EmailConfirmation(string confirmationLink)
    {
        ...
        var subject = $"{ProductName} — Confirme o seu e-mail";
        ...
        var html = $"""
            ...
            <body style="margin:0;padding:24px;background:#f4f4f5;font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1a1a1a;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:8px;padding:32px 28px;box-shadow:0 1px 3px rgba(0,0,0,.08);">
                <tr><td>
                  <p style="margin:0 0 8px;font-size:20px;font-weight:600;">{ProductName}</p>
                  <p style="margin:0 0 20px;font-size:15px;line-height:1.5;">Obrigado por se registar. Confirme o seu endereço de e-mail para ativar a conta e poder iniciar sessão.</p>
                  <p style="margin:0 0 24px;">
                    <a href="{safeLink}" style="display:inline-block;padding:12px 20px;background:#16a34a;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:600;font-size:15px;">Confirmar e-mail</a>
                  </p>
                  <p style="margin:0 0 12px;font-size:13px;color:#525252;line-height:1.5;">Se o botão não funcionar, copie e cole este endereço no navegador:</p>
                  <p style="margin:0 0 24px;font-size:12px;word-break:break-all;color:#16a34a;">{safeLink}</p>
                  <p style="margin:0;font-size:12px;color:#737373;">Se não criou uma conta em {ProductName}, ignore este e-mail.</p>
                </td></tr>
              </table>
            </body>
            </html>
